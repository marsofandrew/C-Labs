...
CPPFLAGS += -Wall -Wextra -Werror -Wno-missing-field-initializers -Wold-style-cast $(if $(BOOST_LOCATION),-I$(BOOST_LOCATION))
CXXFLAGS += -g
...
lab_objects        = $(patsubst %.cpp,out/%.o,$(call lab_sources,$(1)) $(if $(patsubst %/A1,,$(1)),$(call lab_common_sources,$(call student,$(1)))))
...
$(test_objects): out/%.o: %.cpp | $$(@D)/.dir
	$(if $(SILENT),,@echo [C++ ] $<)
	$(hidecmd)$(CXX) $(CPPFLAGS) $(CXXFLAGS) -MMD -MP -c -Wno-unused-parameter -Wno-old-style-cast $(call common_include,$<) -o $@ $<
